...
public class FTUESystem : MonoBehaviour
{
    ...
    public List<Button> boottomButtonsList = new List<Button>();
    ...
    public void DisableButtomButtons()
	{
        if (isTrainingCompleted)
            return;
        for(int i =0;i<boottomButtonsList.Count;i++)
		{
            boottomButtonsList[i].enabled = false;
        }
	}

    public void EnableButtomButtons()
    {
        if (isTrainingCompleted)
            return;
        for (int i = 0; i < boottomButtonsList.Count; i++)
        {
            boottomButtonsList[i].enabled = true;
        }
    }
    ...
    public void Update()
    {
        ...
        switch (currentUISettings.instructionNumber)
        {
            ...
            case 8:
                if (rewardPanel.panelIsActive == true)
                {
                    isActive = false;
                    ChangeState();
    ...
    public void ResetTimeScaleOnHit()
	{
        if (isTrainingCompleted || currentUISettings.instructionNumber!=7)
            return;
        Debug.Log("Play Time");
        Time.timeScale = 1f;
        Time.fixedDeltaTime = Time.timeScale * 0.02f;
